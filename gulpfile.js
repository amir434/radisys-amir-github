  ...
  return gulp
    ...
    .pipe(
      purify([
        "./**/*.twig",
        "./js/*.js",
        "../../core/themes/classy/templates/**/*.twig",
      ])
    )
    .pipe(sourcemaps.init({ loadMaps: true }))
...
gulp.task("purifyit", function () {
  return gulp
    .src("./css/child-theme.min.css")
    .pipe(purify(["./**/*.twig", "./js/*.js"]))
    .pipe(gulp.dest("./dist/"));
});
...
gulp.task("scripts", function () {
  var scripts = [
    ...
    // Sticky Bits
    paths.dev + "/js/stickybits.min.js",
...
gulp.task("copy-assets", function () {
  ...
  var stream = gulp
    .src(paths.node + "cash-dom/dist/cash.min.js")
    .pipe(gulp.dest(paths.dev + "/js/"));

  gulp
    .src(paths.node + "vanilla-lazyload/dist/lazyload.min.js")
    .pipe(gulp.dest(paths.dev + "/js/"));

  gulp
    .src(paths.node + "stickybits/dist/stickybits.min.js")
    .pipe(gulp.dest(paths.dev + "/js/"));
  ...
  // gulp
  //   .src(paths.node + '@fortawesome/fontawesome-pro/webfonts/*.{ttf,woff,woff2,eot,svg}')
  //   .pipe(gulp.dest('./webfonts'));
  //
  // // Copy all Font Awesome 5 SCSS files
  // gulp.src(paths.node + '@fortawesome/fontawesome-pro/scss/*.scss').pipe(gulp.dest(paths.dev + '/sass/fontawesome'));
